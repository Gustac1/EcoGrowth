...
export default function HumidityChartPage() {
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const fetchData = async () => {
                setLoading(true);
                try {
                    const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/Umidade`);
                    ...
                    if (docSnap.exists()) {
                        ...
                        setDesiredHumidity(data.UmidadeDesejada);
                        ...
                        const newDocRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/Umidade`);
                        await setDoc(newDocRef, { UmidadeDesejada: 50, PermissaoPraEditar: false });
                        console.log("Documento criado com valores padrão.");
                        // Defina os valores padrão nos estados
                        setEditPermission(false);
                        setDesiredHumidity(50);
                        setDataLoaded(true);
                    }
                } catch (error) {
    ...
    const handleHumidityChange = (humidity) => {
        const roundedHumidity = Math.round(humidity);
        setDesiredHumidity(roundedHumidity);

        const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/Umidade`);
        setDoc(docRef, { UmidadeDesejada: roundedHumidity }, { merge: true })
            .then(() => {
                console.log("Umidade desejada atualizada com sucesso!");
            })
            .catch((error) => {
                console.error("Erro ao atualizar umidade desejada: ", error);
                Alert.alert(
                    "Erro",
                    "Ocorreu um erro ao tentar atualizar a umidade desejada. Por favor, tente novamente mais tarde.",
                    [{ text: "OK" }]
                );
            });
    };
        ...
        {(!isMember || (isMember && editPermission)) && ( // Verifica se não é um membro ou se é um membro com permissão de edição
            <View>
                <View style={styles.labelContainer}>
                    ...
                    <Text style={styles.label}>Umidade Desejada (%)</Text>
                </View>
                <Slider
                    style={styles.slider}
                    minimumValue={0}
                ...
                <View style={styles.sliderValueContainer}>
                    <Text style={styles.sliderValue}>{desiredHumidity}%</Text>
                </View>
            </View>
        )}
