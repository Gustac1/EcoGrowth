...
import SoilMoistureChart from './SoilMoisture';
...
export default function SoilMoistureChartPage() {
    const route = useRoute();
    const { nomeEstufa, codigoEstufa, userUid } = route.params;
    const [isAdmin, setAdmin] = useState(false);
    const [isMember, setMember] = useState(false);

    const [desiredSoilMoisture, setDesiredSoilMoisture] = useState(50);
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const fetchData = async () => {
                setLoading(true);
                try {
                    const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/UmidadeDoSolo`);
                    ...
                    if (docSnap.exists()) {
                        ...
                        setDesiredSoilMoisture(data.UmidadeDoSoloDesejada);
                        ...
                        const newDocRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/UmidadeDoSolo`);
                        await setDoc(newDocRef, { UmidadeDoSoloDesejada: 50, PermissaoPraEditar: false });
                        console.log("Documento criado com valores padrão.");
                        // Defina os valores padrão nos estados
                        setEditPermission(false);
                        setDesiredSoilMoisture(50);
                        setDataLoaded(true);
                    }
                } catch (error) {
    ...
    const handleSoilMoistureChange = (SoilMoisture) => {
        const roundedSoilMoisture = Math.round(SoilMoisture);
        setDesiredSoilMoisture(roundedSoilMoisture);

        const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/UmidadeDoSolo`);
        setDoc(docRef, { UmidadeDoSoloDesejada: roundedSoilMoisture }, { merge: true })
            .then(() => {
                console.log("UmidadeDoSolo desejada atualizada com sucesso!");
            })
            .catch((error) => {
                console.error("Erro ao atualizar UmidadeDoSolo desejada: ", error);
                Alert.alert(
                    "Erro",
                    "Ocorreu um erro ao tentar atualizar a UmidadeDoSolo desejada. Por favor, tente novamente mais tarde.",
                    [{ text: "OK" }]
                );
            });
    };
        ...
        {(!isMember || (isMember && editPermission)) && ( // Verifica se não é um membro ou se é um membro com permissão de edição
            <View>
                <View style={styles.labelContainer}>
                    ...
                    <Text style={styles.label}>UmidadeDoSolo Desejada (%)</Text>
                </View>
                <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={100}
                    step={1}
                    value={desiredSoilMoisture}
                    onValueChange={handleSoilMoistureChange}
                    minimumTrackTintColor="#000000"
                    maximumTrackTintColor="#000000"
                    thumbTintColor="#000000"
                />
                <View style={styles.sliderValueContainer}>
                    <Text style={styles.sliderValue}>{desiredSoilMoisture}%</Text>
                </View>
            </View>
        )}
