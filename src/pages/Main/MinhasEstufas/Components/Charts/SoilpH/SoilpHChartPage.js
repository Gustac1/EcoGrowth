...
import SoilpHChart from './SoilpHChart';
...
export default function SoilpHChartPage() {
    const route = useRoute();
    const { nomeEstufa, codigoEstufa, userUid } = route.params;
    const [isAdmin, setAdmin] = useState(false);
    const [isMember, setMember] = useState(false);

    const [desiredSoilpH, setDesiredSoilpH] = useState(50);
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const fetchData = async () => {
                setLoading(true);
                try {
                    const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/pHDoSolo`);
                    ...
                    if (docSnap.exists()) {
                        ...
                        setDesiredSoilpH(data.pHDoSoloDesejada);
                        ...
                        const newDocRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/pHDoSolo`);
                        await setDoc(newDocRef, { pHDoSoloDesejada: 50, PermissaoPraEditar: false });
                        console.log("Documento criado com valores padrão para ph.");
    ...
    const handleSoilpHChange = (SoilpH) => {
        const roundedSoilpH = Math.round(SoilpH);
        setDesiredSoilpH(roundedSoilpH);

        const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/pHDoSolo`);
        setDoc(docRef, { pHDoSoloDesejada: roundedSoilpH }, { merge: true })
            .then(() => {
                console.log("pHDoSolo desejada atualizada com sucesso!");
            })
            .catch((error) => {
                console.error("Erro ao atualizar pHDoSolo desejada: ", error);
                Alert.alert(
                    "Erro",
                    "Ocorreu um erro ao tentar atualizar a pHDoSolo desejada. Por favor, tente novamente mais tarde.",
                    [{ text: "OK" }]
                );
            });
    };
        ...
        {(!isMember || (isMember && editPermission)) && ( // Verifica se não é um membro ou se é um membro com permissão de edição
            <View>
                <View style={styles.labelContainer}>
                    ...
                    <Text style={styles.label}>pHDoSolo Desejada (%)</Text>
                </View>
                <Slider
                    style={styles.slider}
                    minimumValue={0}
                    ...
                    step={1}
                    value={desiredSoilpH}
                    onValueChange={handleSoilpHChange}
                    minimumTrackTintColor="#000000"
                    maximumTrackTintColor="#000000"
                    thumbTintColor="#000000"
                />
                <View style={styles.sliderValueContainer}>
                    <Text style={styles.sliderValue}>{desiredSoilpH}%</Text>
                </View>
            </View>
        )}
