...
export default function VentilationChartPage() {
    const route = useRoute();
    const { nomeEstufa, codigoEstufa, userUid } = route.params;
    const [isAdmin, setAdmin] = useState(false);
    const [isMember, setMember] = useState(false);

    const [desiredVentilation, setDesiredVentilation] = useState(50);
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const fetchData = async () => {
                setLoading(true);
                try {
                    const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/Ventilacao`);
                    ...
                    if (docSnap.exists()) {
                        ...
                        setDesiredVentilation(data.VentilacaoDesejada);
                        ...
                        const newDocRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/Ventilacao`);
                        await setDoc(newDocRef, { VentilacaoDesejada: 50, PermissaoPraEditar: false });
                        console.log("Documento criado com valores padrão.");
                        // Defina os valores padrão nos estados
                        setEditPermission(false);
                        setDesiredVentilation(50);
                        setDataLoaded(true);
                    }
                } catch (error) {
    ...
    const handleVentilationChange = (Ventilation) => {
        const roundedVentilation = Math.round(Ventilation);
        setDesiredVentilation(roundedVentilation);

        const docRef = doc(db, `Dispositivos/${codigoEstufa}/Dados/Ventilacao`);
        setDoc(docRef, { VentilacaoDesejada: roundedVentilation }, { merge: true })
            .then(() => {
                console.log("Ventilacao desejada atualizada com sucesso!");
            })
            .catch((error) => {
                console.error("Erro ao atualizar Ventilacao desejada: ", error);
                Alert.alert(
                    "Erro",
                    "Ocorreu um erro ao tentar atualizar a Ventilacao desejada. Por favor, tente novamente mais tarde.",
                    [{ text: "OK" }]
                );
            });
    };
        ...
        {(!isMember || (isMember && editPermission)) && ( // Verifica se não é um membro ou se é um membro com permissão de edição
            <View>
                <View style={styles.labelContainer}>
                    ...
                    <Text style={styles.label}>Ventilacao Desejada (%)</Text>
                </View>
                <Slider
                    style={styles.slider}
                    minimumValue={0}
                    ...
                    onValueChange={handleVentilationChange}
                    minimumTrackTintColor="#000000"
                    maximumTrackTintColor="#000000"
                    thumbTintColor="#000000"
                />
                <View style={styles.sliderValueContainer}>
                    <Text style={styles.sliderValue}>{desiredVentilation}%</Text>
                </View>
            </View>
        )}
